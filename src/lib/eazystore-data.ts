...
export const TEMPLATES: { id: TemplateId; name: string; tagline: string; gradient: string; accent: string; category: string; premium?: boolean }[] = [
  ...
  { id: "quickmart", name: "QuickMart Slider", tagline: "Teal marketplace with sliding Categories and sliding Top Products rows, dense multi-section grids, and promo banners", gradient: "from-teal-500 to-emerald-400", accent: "#0d9488", category: "General", premium: true },
    ...
    queryFn: async (): Promise<{ store: StoreRow; product: ProductRow; imageUrls: string[] } | null> => {
      ...
      const { data: product, error: pErr } = await supabase
        .from("products").select(
          "id, store_id, name, price, regular_price, stock, status, image_url, gallery_urls, category_id, brand, sku, warranty, description, short_description, video_url, product_serial, unit_name, delivery_charge, default_delivery_charge, specific_delivery_charges, initial_sold_count, is_out_of_stock, created_at, updated_at",
        )
