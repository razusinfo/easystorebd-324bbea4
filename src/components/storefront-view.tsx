...
export function StorefrontView({ slug }: { slug: string }) {
  ...
  if (s.template === "flipmart" || s.template === "megamart" || s.template === "trendmart" || s.template === "shopii" || s.template === "quickmart") {
    return (
      <FlipmartTemplate
        store={s}
        products={orderedProducts}
